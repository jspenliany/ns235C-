...
TAVRagent::new_SvehiUpdate_selfinfo(){
	...
	double delta_x = node_->dX();
	double delta_y = node_->dY();
	new_Supdate_JuncInfo(node_->X(),node_->Y());//bid info
	new_Supdate_DirectionInfo(delta_x,delta_y);//orient info
	...
	fprintf(stdout,"\n-----check valid--xy=%8.4f,%8.4f delta=%4.2f,%4.2f bidcpn=%4d,%4d,%4d %8.4f",S_vehicle_position_x_,S_vehicle_position_y_,
											delta_x,delta_y,S_currentBID,S_loc_BID_previous,S_loc_BID_toward,Scheduler::instance().clock());
}
/**
 * find driving direction of self, but street info must be confirmed
 * */
u_int8_t
TAVRagent::new_Supdate_DirectionInfo(double deltaX, double deltaY){
