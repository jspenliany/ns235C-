...
*/
		sprintf(tmpStr,"---***---recv route manager  sip=%s cip=%s ",
					Address::instance().print_nodeaddr(ih->saddr()),
					Address::instance().print_nodeaddr(vehicle_ip()));
...
//		if((recv_seqno_List_[i] == 0)) continue;// own the fresh data
		if(timeValid_compare(ch->timestamp(),vehicle_info_updateTime_list()[i])) continue;
		char ipStr[15] = {'\0'};
...
//		fprintf(stdout," -recv_Wired- ipStr=%s -*- ",ipStr);
	...
	/*	//to app
		...
		for(int i=0; i< current_veh_num; i++){
			...
			if(Address::instance().get_lastaddr(vehicle_ip_LIST_[i]) != i) continue;//no info recv

			appInfo->vehicle_id_list[i] = i;
			appInfo->vehicle_position_x_list[i] = vehicle_position_x_LIST_[i];
			appInfo->vehicle_position_y_list[i] = vehicle_position_y_LIST_[i];
			appInfo->vehicle_speed_list[i] = vehicle_speed_LIST_[i];
			appInfo->recv_seqno_list[i] = recv_seqno_List_[i];
		...
		appih->dport() = 2;
		appih->dport() = 50;port_TAVR_App
		appih->dport() = port_TAVR_App;
