...
#define	NEW_CURRENT_TIME	Scheduler::instance().clock()
...
void
TAVR_vehiSinfoBcastTimer::expire(Event* e) {
//	if(Address::instance().get_firstaddr(agent_->vehicle_ip()) == NEW_TIMER_BASE)return;
//	fprintf(stdout,"\nself updating following......");
	if(agent_->new_S_node_active() && agent_->new_S_selfUpdate_active())
		agent_->new_update_selfInfo();

	agent_->new_reset_vehi_self_infoTimer();
}
...
//	if(Address::instance().get_firstaddr(agent_->vehicle_ip()) == NEW_TIMER_BASE)return;
	...
	if(agent_->new_S_node_active() && agent_->new_S_globalUpdae_active())
...
//	if(Address::instance().get_firstaddr(agent_->vehicle_ip()) == NEW_TIMER_BASE)return;
	...
	if(agent_->new_S_node_active() && agent_->new_S_localUpdate_active())
...
//	if(Address::instance().get_firstaddr(agent_->vehicle_ip()) == NEW_TIMER_VEHI)return;
	...
	if(agent_->new_S_node_active() && agent_->new_S_baseUpdate_active())
		agent_->new_update_BaselocalInfo();
...
double TAVRagent::Gneighbor_radius = 0.0;
...
int32_t	TAVRagent::LG_NEW_id_uT[NEW_LG_LINKS_MAX][NEWG_VEHI_NUM];
int32_t	TAVRagent::LG_NEW_speed_direction[NEW_LG_LINKS_MAX][NEWG_VEHI_NUM];
int32_t	TAVRagent::LG_NEW_locX[NEW_LG_LINKS_MAX][NEWG_VEHI_NUM];
int32_t	TAVRagent::LG_NEW_locY[NEW_LG_LINKS_MAX][NEWG_VEHI_NUM];
int16_t	TAVRagent::LG_NEW_loc_BID_previous[NEW_LG_LINKS_MAX][NEWG_VEHI_NUM];
int16_t	TAVRagent::LG_NEW_loc_BID_toward[NEW_LG_LINKS_MAX][NEWG_VEHI_NUM];
...
TAVRagent::TAVRagent(nsaddr_t ip) : Agent(PT_SIMUTAVR), tavr_pkttimer_(this), tavr_protimer_(this),
		S_vehiSinfoBcastTimer(this),S_vehiGinfoBcastTimer(this),S_vehiLinfoBcastTimer(this),S_baseLinfoBcastTimer(this){
	vehicle_ip_ = ip;
	vehicle_speed_ = -10;
	hello_amount_ = 0;
...
//-----------------------------------------
	...
	S_vehicle_ip_ = ip;
    S_vehi_End_Time_flag = false;
	for (int new_i=0; new_i < NEWL_VEHI_NUM; new_i++) {
		L_idTOindex[new_i] = -1;
	}

	S_timer_slocal_RUN_flag = false;
...
//	fprintf(stdout, "___TAVRagent::recv__");
	struct hdr_cmn 			*ch = HDR_CMN(p);
	struct hdr_ip			*ih = HDR_IP(p);
	char					tmpStr[260];

	if(!std_NETWORK_flag)return;
//	fprintf(stdout, "    I'm here, check point across");
...
//	fprintf(stdout, "    I'm here, check point across----2   ");
		if(debug_file  && (Scheduler::instance().clock() > file_wirte_Time + 0.001)){
...
int TAVRagent::command(int argc, const char*const* argv) {
	...
	//				rtable_.print(logtarget_);
			...
			init_juncInfo();
			return TCL_OK;
		}else if (strcasecmp (argv[1], "send_Rmsg") == 0) {//old info update timer--------------abandoned
		...
		else if(strcasecmp(argv[1], "vehiSelf_Bcast") == 0){
			S_vehiSinfoBcastTimer.resched(0.0);
			return (TCL_OK);
...
//------------------------------------------------------------------------
			...
			return (TCL_OK);
		}else if(strcmp(argv[1], "gNBRadius") == 0) {
			Gneighbor_radius = atof(argv[2]);
			return (TCL_OK);
		}else if(strcmp(argv[1], "gscenVehiNum") == 0) {
			Gcurrent_vnum = atoi(argv[2]);
			return (TCL_OK);
		}else if(strcmp(argv[1], "stimerSlocalperiod") == 0) {
			S_timer_slocal_Binterval = atof(argv[2]);
			return (TCL_OK);
			...
			return (TCL_OK);
		}else if(strcmp(argv[1], "stimerSlocalflag") == 0) {
			S_timer_slocal_RUN_flag = (strcmp(argv[2],"true"))?false:true;
//			if(S_timer_slocal_RUN_flag)fprintf(stdout,"value is true");
//			else fprintf(stdout,"value is false");
			return (TCL_OK);
		}else if(strcmp(argv[1], "stimerVlocalflag") == 0) {
			S_timer_vlocal_RUN_flag = (strcmp(argv[2],"true"))?false:true;
			return (TCL_OK);
		}else if(strcmp(argv[1], "stimerVglobalflag") == 0) {
			S_timer_vglobal_RUN_flag = (strcmp(argv[2],"true"))?false:true;
			return (TCL_OK);
		}else if(strcmp(argv[1], "stimerBlocalflag") == 0) {
			S_timer_blocal_RUN_flag = (strcmp(argv[2],"true"))?false:true;
			return (TCL_OK);
		}else if(strcmp(argv[1], "sNode_runflag") == 0) {
			S_node_RUN_flag = (strcmp(argv[2],"true"))?false:true;
			
			if(!S_node_RUN_flag){//here clear the info stored in LG etc variables
				new_clear_trafficInfo__stopFunc();
				S_vehi_End_Time_flag = true;
			}
...
//==============-----------------------=======----------------------------
void
TAVRagent::new_reset_vehi_self_infoTimer(){
	S_vehiSinfoBcastTimer.resched(S_timer_slocal_Binterval);
}
...
void
TAVRagent::new_update_selfInfo(){
	new_SvehiUpdate_selfinfo();
}
...
void
TAVRagent::new_update_BaselocalInfo(){
...
void
TAVRagent::new_SvehiUpdate_globalinfo(){
	//new_Supdate_selfInfo();
}

void
TAVRagent::new_SvehiUpdate_selfinfo(){
	char tmpStr[400];
	int nodeid = Address::instance().get_lastaddr(vehicle_ip());
	double	cTIME = NEW_CURRENT_TIME;
	...
	double delta_x = node_->dX();
	double delta_y = node_->dY();
	new_SupdateSI_JuncInfo(node_->X(),node_->Y());//bid info
	new_SupdateSI_DirectionInfo(delta_x,delta_y);//orient info
	S_vehicle_position_x_ = node_->X(); //current position for instant hello msg
	S_vehicle_position_y_= node_->Y();//
	S_vehicle_speed_= node_->speed(); //

	//update LG_XXXX[] for local info update
	int		TMP_axis_scale=NEWG_SCALE_AXIS;
	int		TMP_speed_scale=NEWG_SCALE_SPEED;
	int32_t	tmp_ID = nodeid<<NEWG_COUPLE_VID;
	int32_t tmp_TIME = ((int)(cTIME*10000))&NEWG_DECOUPLE_UPTIME;

	int32_t	tmp_SPEED = ((int)(S_vehicle_speed_*TMP_speed_scale))<<NEWG_COUPLE_VSPEED;
	int32_t tmp_DIRECTION = S_vehicle_direction_;

	int32_t	tmp_LOCX = ((int)(S_vehicle_position_x_*TMP_axis_scale))&NEWG_DECOUPLE_AXIS;
	int32_t tmp_LOCY = ((int)(S_vehicle_position_y_*TMP_axis_scale))&NEWG_DECOUPLE_AXIS;

	int32_t	tmp_BID_PREVIOUS = S_loc_BID_previous;
	int32_t tmp_BID_TOWARD = S_loc_BID_toward;

	LG_id_uT[nodeid] = tmp_ID + tmp_TIME;
	LG_speed_direction[nodeid] = tmp_SPEED + tmp_DIRECTION;
	LG_locX[nodeid] = tmp_LOCX;
	LG_locY[nodeid] = tmp_LOCY;
	LG_loc_BID_previous[nodeid] = tmp_BID_PREVIOUS;
	LG_loc_BID_toward[nodeid] = tmp_BID_TOWARD;

	fprintf(stdout,"\n--id=%d---check valid--xy=%8.4f,%8.4f delta=%4.2f,%4.2f bidcpn=%4d,%4d,%4d UPDATE*%4d %4d %4d %4d %4d %4d %4d %4d ss %4d %4d %4d %4d %4d %4d  %8.4f",
			nodeid,S_vehicle_position_x_,S_vehicle_position_y_,delta_x,delta_y,S_currentBID,S_loc_BID_previous,S_loc_BID_toward,
			tmp_ID,tmp_TIME,tmp_SPEED,tmp_DIRECTION,tmp_LOCX,tmp_LOCY,tmp_BID_PREVIOUS,tmp_BID_TOWARD,
			LG_id_uT[nodeid],LG_speed_direction[nodeid],LG_locX[nodeid],LG_locY[nodeid],LG_loc_BID_previous[nodeid],LG_loc_BID_toward[nodeid],cTIME);
}
/**
 * find driving direction of self, but street info must be confirmed
 * */
u_int8_t
TAVRagent::new_SupdateSI_DirectionInfo(double deltaX, double deltaY){
	if(fabs(deltaX) + fabs(deltaY) < 0.01){//not start running
		if(Stmp_loc_BID_previous!= S_currentBID){

		}else{
			if(Stmp_loc_BID_toward - Stmp_loc_BID_previous ==1){
				S_vehicle_direction_ = NEW_ORIENT_RIGHT;
			}else if(Stmp_loc_BID_toward - Stmp_loc_BID_previous ==-1){
				S_vehicle_direction_ = NEW_ORIENT_LEFT;
			}else if(Stmp_loc_BID_toward - Stmp_loc_BID_previous ==Gscen_colC){
				S_vehicle_direction_ = NEW_ORIENT_UP;
			}else if(Stmp_loc_BID_toward - Stmp_loc_BID_previous ==-Gscen_colC){
				S_vehicle_direction_ = NEW_ORIENT_DOWN;
			}else {
				fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  Zero delta info %0.4f %0.4f is BAD",deltaX,deltaY);
				exit(1);
			}
			S_loc_BID_toward 	= Stmp_loc_BID_toward;
			S_loc_BID_previous	= Stmp_loc_BID_previous;
		}
	}else if(fabs(deltaX) + fabs(deltaY) > 1.7){
		fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info %0.4f %0.4f is overflow",deltaX,deltaY);
		exit(1);
	}else if(deltaX > 0.51 && (deltaX > fabs(deltaY)+0.1)){//left OR right-----right
		if(Stmp_loc_BID_previous!= S_currentBID){//more than one street are valid, need more checks with delta infos-----------NO update with bid infos
			if(Stmp_loc_BID_previous==-1 && Stmp_loc_BID_toward ==-1){//maintains last BIDs-

			}else{//three BIDs are provided
				//toward----S_currentBID
				if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){//master orientation

				}else if(S_vehicle_direction_ == NEW_ORIENT_LEFT){
					fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to RIGHT but history is LEFT  conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}else if(S_vehicle_direction_ == NEW_ORIENT_UP){

				}else if(S_vehicle_direction_ == NEW_ORIENT_DOWN){

				}else {//the first movement
					fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to RIGHT but history Orient is NONE  conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}
				//leave-----S_currentBID
			}
		}else{
			if(Stmp_loc_BID_previous== Stmp_loc_BID_toward){//------------------no orientation is provided
				fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is no orientation is provided ");
				exit(1);
			}else if(Stmp_loc_BID_previous - Stmp_loc_BID_toward ==1){//at left
				S_loc_BID_toward 	= Stmp_loc_BID_previous;
				S_loc_BID_previous	= Stmp_loc_BID_toward;
				S_vehicle_direction_ = NEW_ORIENT_RIGHT;
			}else if(Stmp_loc_BID_previous - Stmp_loc_BID_toward ==-1){//at right
				S_loc_BID_toward 	= Stmp_loc_BID_toward;
				S_loc_BID_previous	= Stmp_loc_BID_previous;
				S_vehicle_direction_ = NEW_ORIENT_RIGHT;
			}else if(Stmp_loc_BID_previous - Stmp_loc_BID_toward ==Gscen_colC){//at down....lock the direction info
				S_loc_BID_toward 	= Stmp_loc_BID_previous;
				S_loc_BID_previous	= Stmp_loc_BID_toward;
				S_vehicle_direction_ = NEW_ORIENT_UP;
			}else if(Stmp_loc_BID_previous - Stmp_loc_BID_toward ==-Gscen_colC){//at up....lock the direction info
				S_loc_BID_toward 	= Stmp_loc_BID_previous;
				S_loc_BID_previous	= Stmp_loc_BID_toward;
				S_vehicle_direction_ = NEW_ORIENT_DOWN;
			}else {
				fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to RIGHT but bid-tmp=%d %d are Wired",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
				exit(1);
			}
		}

	}else if(deltaX < -0.51 && (-deltaX > fabs(deltaY)+0.1)){//left OR right--------------to left
		if(Stmp_loc_BID_previous!= S_currentBID){//more than one street are valid, need more checks with delta infos-----------NO update with bid infos
			if(Stmp_loc_BID_previous==-1 && Stmp_loc_BID_toward ==-1){//maintains last BIDs-

			}else{//three BIDs are provided
				//toward----S_currentBID
				if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){
					fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to LEFT but history is RIGHT  conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}else if(S_vehicle_direction_ == NEW_ORIENT_LEFT){//master orientation
					...
					fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to LEFT but history Orient is NONE  conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}
				//leave-----S_currentBID
			}
			...
			if(Stmp_loc_BID_previous== Stmp_loc_BID_toward){//------------------no orientation is provided
				...
				fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to LEFT but bid-tmp=%d %d are Wired",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
				exit(1);
			}
		}
	}else if(deltaY > 0.51 && (deltaY > fabs(deltaX)+0.1)){//up OR down---------------to up
		if(Stmp_loc_BID_previous!= S_currentBID){//more than one street are valid, need more checks with delta infos-----------NO update with bid infos
			if(Stmp_loc_BID_previous==-1 && Stmp_loc_BID_toward ==-1){
				//maintains last BIDs-
			}else{//three BIDs are provided
				if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){
				...
				}else if(S_vehicle_direction_ == NEW_ORIENT_UP){//master orientation

				}else if(S_vehicle_direction_ == NEW_ORIENT_DOWN){
					fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to UP but history is DOWN conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}else {//the first movement
					fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to UP but history Orient is NONE conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}
				//leave-----S_currentBID
			}
			...
			if(Stmp_loc_BID_previous== Stmp_loc_BID_toward){//------------------no orientation is provided
				fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is no orientation is provided ");
				exit(1);
			}else if(Stmp_loc_BID_previous - Stmp_loc_BID_toward ==1){//at left....lock the direction info
				S_loc_BID_toward 	= Stmp_loc_BID_previous;
				S_loc_BID_previous	= Stmp_loc_BID_toward;
				S_vehicle_direction_ = NEW_ORIENT_RIGHT;
			}else if(Stmp_loc_BID_previous - Stmp_loc_BID_toward ==-1){//at right....lock the direction info
				S_loc_BID_toward 	= Stmp_loc_BID_previous;
				S_loc_BID_previous	= Stmp_loc_BID_toward;
				S_vehicle_direction_ = NEW_ORIENT_LEFT;
			}else if(Stmp_loc_BID_previous - Stmp_loc_BID_toward ==Gscen_colC){//at down---------enter current BID
				S_loc_BID_toward 	= Stmp_loc_BID_previous;
				S_loc_BID_previous	= Stmp_loc_BID_toward;
				S_vehicle_direction_ = NEW_ORIENT_UP;
			}else if(Stmp_loc_BID_previous - Stmp_loc_BID_toward ==-Gscen_colC){//at up---------leave current BID
				S_loc_BID_toward 	= Stmp_loc_BID_toward;
				S_loc_BID_previous	= Stmp_loc_BID_previous;
				S_vehicle_direction_ = NEW_ORIENT_UP;
			}else {
				fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to UP but bid-tmp=%d %d are Wired",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
				exit(1);
			}
		}
	}else if(deltaY < -0.51 && (-deltaY > fabs(deltaX)+0.1)){//up OR down
		if(Stmp_loc_BID_previous!= S_currentBID){//more than one street are valid, need more checks with delta infos-----------NO update with bid infos
			if(Stmp_loc_BID_previous==-1 && Stmp_loc_BID_toward ==-1){//BAD-----
				//maintains last BIDs-
			}else{//three BIDs are provided
				if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){
					...
					fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to DOWN but history is UP conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}else if(S_vehicle_direction_ == NEW_ORIENT_DOWN){

				}else {//the first movement
					fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to DOWN but history Orient is NONE conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}
				//leave-----S_currentBID
			}
			...
			if(Stmp_loc_BID_previous== Stmp_loc_BID_toward){//------------------no orientation is provided
				...
				fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info is to DOWN but bid-tmp=%d %d are Wired",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
				exit(1);
			}
		}
	}else{//at junction area, but the difference between X-Y axis delta infos is less than 0.1
		if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){
			...
			fprintf(stderr,"TAVRagent::new_SupdateSI_DirectionInfo  delta info %0.4f %0.4f can NOT be dealt but history Orient is NONE conflicts!!!%d %d",deltaX,deltaY,Stmp_loc_BID_previous,Stmp_loc_BID_toward);
			exit(1);
		}
	}
 ...
 * */
void
TAVRagent::new_SupdateSI_JuncInfo(double cvx, double cvy){
	...
	//-------------------test for memory storage effective
		for(int i=0; i<300; i++){
			for(int j=0; j<2048; j++){
				LG_NEW_loc_BID_previous[i][j] = i;
				LG_NEW_loc_BID_previous[i][j] = i;
				LG_NEW_id_uT[i][j] = i;
				LG_NEW_speed_direction[i][j] = i;
			}
		}

	}else{
		new_SpotentialSI_Junc(S_currentBID,S_loc_BID_previous,S_loc_BID_toward);//S_potentialBaseIDlist save the result
//		fprintf(stdout,"\n-----------------------------\n");
		for(int pBindex=0; pBindex<sizeof(Stmp_potentialBaseIDlist)/sizeof(Stmp_potentialBaseIDlist[0]); pBindex++){
			if(Stmp_potentialBaseIDlist[pBindex]<0){
//				fprintf(stdout,"TAVRagent::new_SupdateSI_JuncInfo vehi-%d bid=%d %d %d has %d potential junction",nodeid,S_currentBID,S_loc_BID_previous,S_loc_BID_toward,pBindex);
				break;
			}
			int16_t		Bri = Stmp_potentialBaseIDlist[pBindex]/Gscen_colC;
			int16_t		Bci = Stmp_potentialBaseIDlist[pBindex]%Gscen_colC;
			if(tmp_Distance > (GXbas_LOC[Bri][Bci]-cvx)*(GXbas_LOC[Bri][Bci]-cvx) + (GYbas_LOC[Bri][Bci]-cvy)*(GYbas_LOC[Bri][Bci]-cvy)){
				tmp_BID = Bri*Gscen_colC + Bci;
				tmp_Distance = (GXbas_LOC[Bri][Bci]-cvx)*(GXbas_LOC[Bri][Bci]-cvx) + (GYbas_LOC[Bri][Bci]-cvy)*(GYbas_LOC[Bri][Bci]-cvy);
			}
//			fprintf(stdout,"\n----%d %6.2f %d %6.2f",Stmp_potentialBaseIDlist[pBindex],
//					(GXbas_LOC[Bri][Bci]-cvx)*(GXbas_LOC[Bri][Bci]-cvx) + (GYbas_LOC[Bri][Bci]-cvy)*(GYbas_LOC[Bri][Bci]-cvy),
//					tmp_BID,tmp_Distance);
		}
		if(tmp_BID>-1){
			S_currentBID = tmp_BID;
			if(tmp_Distance < Gjunc_radius*Gjunc_radius){
...
//		fprintf(stdout,"\n-----------------------------\n");
	...
	bool tmp_UP = new_ScheckSI_street(cvx,cvy,NEW_ORIENT_UP);
	bool tmp_DOWN = new_ScheckSI_street(cvx,cvy,NEW_ORIENT_DOWN);
	bool tmp_LEFT = new_ScheckSI_street(cvx,cvy,NEW_ORIENT_LEFT);
	bool tmp_RIGHT = new_ScheckSI_street(cvx,cvy,NEW_ORIENT_RIGHT);

	Stmp_loc_BID_previous = -1;
	Stmp_loc_BID_toward = -1;


	if(tmp_UP && tmp_LEFT){//enter into overlap area, no action takes
		if(S_junction_){
			Stmp_loc_BID_previous = S_currentBID + Gscen_colC;
			Stmp_loc_BID_toward = S_currentBID - 1;
		}else{
			//has history record?
			fprintf(stdout,"\nTAVRagent::new_SupdateSI_JuncInfo vehi-%d bid=%d %d %d has LU potential junctions",nodeid,S_currentBID,S_loc_BID_previous,S_loc_BID_toward);
		}
		...
		if(S_junction_){
			...
			//has history record?
			fprintf(stdout,"\nTAVRagent::new_SupdateSI_JuncInfo vehi-%d bid=%d %d %d has RU potential junctions",nodeid,S_currentBID,S_loc_BID_previous,S_loc_BID_toward);
		}
		...
		if(S_junction_){
			...
			//has history record?
			fprintf(stdout,"\nTAVRagent::new_SupdateSI_JuncInfo vehi-%d bid=%d %d %d has LD potential junctions",nodeid,S_currentBID,S_loc_BID_previous,S_loc_BID_toward);
		}
		...
		if(S_junction_){
			...
			//has history record?
			fprintf(stdout,"\nTAVRagent::new_SupdateSI_JuncInfo vehi-%d bid=%d %d %d has RD potential junctions",nodeid,S_currentBID,S_loc_BID_previous,S_loc_BID_toward);
		}
	}else if(tmp_UP){
		Stmp_loc_BID_previous = S_currentBID;
		Stmp_loc_BID_toward = Stmp_loc_BID_previous + Gscen_colC;
	}else if(tmp_DOWN){
		Stmp_loc_BID_previous = S_currentBID;
		Stmp_loc_BID_toward = Stmp_loc_BID_previous - Gscen_colC;
	}else if(tmp_LEFT){
		Stmp_loc_BID_previous = S_currentBID;
		Stmp_loc_BID_toward = Stmp_loc_BID_previous - 1;
	}else if(tmp_RIGHT){
		Stmp_loc_BID_previous = S_currentBID;
		Stmp_loc_BID_toward = Stmp_loc_BID_previous + 1;
	}else{//loop itself
/*		S_currentBID = tmp_SaveBID%Gscen_colC + 1 + tmp_SaveBID - tmp_SaveBID%Gscen_colC;
		...
		if(S_currentBID > -1) new_Supdate_JuncInfo(cvx,cvy);*/
		if(!S_junction_){
			if(S_vehicle_direction_ == NEW_ORIENT_RIGHT || S_vehicle_direction_ == NEW_ORIENT_LEFT || S_vehicle_direction_ == NEW_ORIENT_UP || S_vehicle_direction_ == NEW_ORIENT_DOWN){//this condition check the info of "S_loc_BID_previous" & S"loc_BID_toward"
			}else{
				fprintf(stderr,"\nTAVRagent::new_SupdateSI_JuncInfo finish vehi-%d %0.2f %0.2f bid=%d %d %d UDLR=%d %d %d %d",nodeid,cvx,cvy,S_currentBID,Stmp_loc_BID_previous,Stmp_loc_BID_toward,tmp_UP,tmp_DOWN,tmp_LEFT,tmp_RIGHT);
				exit(1);
			}
		}else{//inner area????
/*			S_loc_BID_previous = S_currentBID;
			S_loc_BID_toward = S_currentBID;*/
			Stmp_loc_BID_previous = S_currentBID;
			Stmp_loc_BID_toward = Stmp_loc_BID_previous;
		}

	}
	fprintf(stdout,"\nTAVRagent::new_SupdateSI_JuncInfo finish vehi-%d bid=%d %d %d UDLR=%d %d %d %d",nodeid,S_currentBID,Stmp_loc_BID_previous,Stmp_loc_BID_toward,tmp_UP,tmp_DOWN,tmp_LEFT,tmp_RIGHT);
}

/**
 * which street does current car located?, potential direction TYPE is used to detect
 * */
bool
TAVRagent::new_ScheckSI_street(double cvx, double cvy, int Orient_type){
	double  tmp_Slope_prevBasenextBase = 0.0;//k
	double	tmp_Margin_prevBasenextBase = 0.0;//b   y=kx+b
	double  tmp_Jitter_laneEdgeLine	= 0.0;//always positive
	...
	int sBr = S_currentBID/Gscen_colC;
	int sBc = S_currentBID%Gscen_colC;
	int eBr = -1;
	int eBc = -1;
...
//	double 	innerSquarewidth = 3.2 / 1.413;
	if(Orient_type == NEW_ORIENT_UP && sBr>Gscen_rowC-2){
		return false;
	}else if(Orient_type == NEW_ORIENT_DOWN && sBr<1){
		return false;
	}
	if(Orient_type == NEW_ORIENT_RIGHT && sBc>Gscen_colC-2){
		return false;
	}else if(Orient_type == NEW_ORIENT_LEFT && sBc<1){
		return false;
	}

	if(Orient_type == NEW_ORIENT_UP){
		eBr = S_currentBID/Gscen_colC+1;
		eBc = sBc;
		...
		eBr = sBr;
		eBc = S_currentBID%Gscen_colC+1;
	}else if(Orient_type == NEW_ORIENT_LEFT){
		eBr = sBr;
		eBc = S_currentBID%Gscen_colC-1;
	}else{
		fprintf(stderr,"TAVRagent::new_ScheckSI_street  potential street info error. No one of the four");
		exit(1);
	}

	if((cvx < GXbas_LOC[sBr][sBc]+0.001) && (Orient_type == NEW_ORIENT_RIGHT)){//no RIGHT street
		return false;
	}else if((cvx > GXbas_LOC[sBr][sBc]-0.001) && (Orient_type == NEW_ORIENT_LEFT)){//no LEFT street
		return false;
	}
	if((cvy < GYbas_LOC[sBr][sBc]+0.001) && (Orient_type == NEW_ORIENT_UP)){//no UP street
		return false;
	}else if((cvy > GYbas_LOC[sBr][sBc]-0.001) && (Orient_type == NEW_ORIENT_DOWN)){//no DOWN street
		return false;
	}


	int startBASE = sBr*Gscen_colC + sBc;
	int endBASE = eBr*Gscen_colC + eBc;
	if(fabs(GXbas_LOC[eBr][eBc] - GXbas_LOC[sBr][sBc]) < 2.0){//same column, of regular grid
		...
		if(eBc-sBc){//must same row
			if(endBASE < startBASE){
				tmp_Slope_prevBasenextBase = (GYbas_LOC[sBr][sBc] - GYbas_LOC[eBr][eBc])/(GXbas_LOC[sBr][sBc] - GXbas_LOC[eBr][eBc]);
				tmp_Margin_prevBasenextBase = GYbas_LOC[eBr][eBc] - tmp_Slope_prevBasenextBase*GXbas_LOC[eBr][eBc];
			}else{
...
/*			tmp_Slope_prevBasenextBase = (GYbas_LOC[eBr][eBc] - GYbas_LOC[sBr][sBc])/(GXbas_LOC[eBr][eBc] - GXbas_LOC[sBr][sBc]);
			...
			fprintf(stdout,"\n TYPE=%d k=%0.2f b=%0.2f vx=%0.2f vy=%0.2f maxy=%0.2f miny=%0.2f",Orient_type,
		...
		if(tmp_Check_valid){
			fprintf(stderr,"TAVRagent::new_ScheckSI_street  the pair of %d-%d or %d-%d is error, since only one pair is zero",eBc,sBc,eBr,sBr);
			exit(1);
		}else{

			if(directionUP_flag && directionLEFT_flag){
				fprintf(stderr,"TAVRagent::new_ScheckSI_street both direction info can be induced  , %d %d %0.2f %0.2f",startBASE,endBASE, cvx, cvy);
				exit(1);
			}else if(directionLEFT_flag){
				if(endBASE == startBASE-1) return true;
				else if(endBASE == startBASE+1) return true;
...
void
TAVRagent::new_SpotentialSI_Junc(int cBID,int pBID,int tBID){
	int 	tmp_BaseNum = Gscen_rowC*Gscen_colC;
	int		potentialCheckBase[tmp_BaseNum];
	bool	tmp_check_valid = true;
	for(int i=0; i<tmp_BaseNum; i++){
		potentialCheckBase[i] = 0;
		Stmp_potentialBaseIDlist[i%39]=-1;
	}


	if(cBID <0){
		fprintf(stderr,"TAVRagent::new_SpotentialSI_Junc S_currentBID should have positive value ");
		exit(1);
	}else{
	...
	if(tBID<0){
		...
		if(!tmp_check_valid){
			fprintf(stderr,"TAVRagent::new_SpotentialSI_Junc pBID has Negative value but tBID has positive value ");
			exit(1);
		}
	...
	for(int i=0; i<tmp_BaseNum; i++){
		if(potentialCheckBase[i])//  >0????
			Stmp_potentialBaseIDlist[pIndex++]=i;
	}
}

/**
 * update the neighbors info of current vehicle,-----------simulates hello message
 *	the threshold of radius will be less than Communication radius--------------------for 
 * */
void
TAVRagent::new_ScurrentVLI_neighbors(){
	if(Gneighbor_radius < 100.1 || Gneighbor_radius> 250.1){
		fprintf(stderr,"\n-TAVRagent::new_ScurrentVLI_neighbors-----the radius of checking neighbor is INVALID");
		exit(1);
	}
	...
	for(int LG_index=0; LG_index < NEWL_VEHI_NUM; LG_index++){
		L_idTOindex[LG_index] = -1;
		L_id_uT[LG_index] = -1;
		L_speed_direction[LG_index] = -1;
		L_loc_BID_previous[LG_index] = -1;
		L_loc_BID_toward[LG_index] = -1;
		L_locX[LG_index] = -1;
		L_locY[LG_index] = -1;
	}
	L_nextIndex = 0;

	double	tmp_vehi_X = 0.0;
	double	tmp_vehi_Y = 0.0;
	int		tmp_vehi_ID = -1;

	for(int LG_index=0; LG_index < Gcurrent_vnum; LG_index++){
		
		if(LG_index == nodeid ) continue;
		if(LG_id_uT[LG_index] < 0) continue;
		
		tmp_vehi_X = LG_locX[LG_index]*1.0/NEWG_SCALE_AXIS;
		tmp_vehi_Y = LG_locY[LG_index]*1.0/NEWG_SCALE_AXIS;

		if(((tmp_vehi_X - S_vehicle_position_x_)*(tmp_vehi_X - S_vehicle_position_x_) + (tmp_vehi_Y - S_vehicle_position_y_)*(tmp_vehi_Y - S_vehicle_position_y_)) < Gneighbor_radius*Gneighbor_radius+1.0){
			tmp_vehi_ID = (LG_id_uT[LG_index]&NEWG_DECOUPLE_VID)>>NEWG_COUPLE_VID;
			L_idTOindex[L_nextIndex] = tmp_vehi_ID;
			L_id_uT[L_nextIndex] = LG_id_uT[LG_index];
			L_speed_direction[L_nextIndex] = LG_speed_direction[LG_index];
			L_loc_BID_previous[L_nextIndex] = LG_loc_BID_previous[LG_index];
			L_loc_BID_toward[L_nextIndex] = LG_loc_BID_toward[LG_index];
			L_locX[L_nextIndex] = LG_locX[LG_index];
			L_locY[L_nextIndex] = LG_locY[LG_index];
			L_nextIndex++;
			fprintf(stdout,"\n-TAVRagent::new_ScurrentVLI_neighbors--- %d %6.2f %6.2f %d %6.2f %6.2f",
					nodeid,S_vehicle_position_x_,S_vehicle_position_y_,
					tmp_vehi_ID,tmp_vehi_X,tmp_vehi_Y);
		}
	}
...
/**
  * each vehicle will erase info in local&self&global arraies and stop function of receiving data
  */
void
TAVRagent::new_clear_trafficInfo__stopFunc(){
	//clear data
	int LG_index = Address::instance().get_lastaddr(vehicle_ip());
	LG_id_uT[LG_index]=-100;
	L_speed_direction[L_nextIndex] = 0;
	L_loc_BID_previous[L_nextIndex] = -100;
	L_loc_BID_toward[L_nextIndex] = -100;
	L_locX[L_nextIndex] = -100;
	L_locY[L_nextIndex] = -100;
	//stop function flags
}
