...
int32_t	TAVRagent::LG_NEW_id_uT[NEWG_VEHI_NUM]={};
int32_t	TAVRagent::LG_NEW_speed_direction[NEWG_VEHI_NUM]={};
int32_t	TAVRagent::LG_NEW_locX[NEWG_VEHI_NUM]={};
int32_t	TAVRagent::LG_NEW_locY[NEWG_VEHI_NUM]={};
int16_t	TAVRagent::LG_NEW_loc_BID_previous[NEWG_VEHI_NUM]={};
int16_t	TAVRagent::LG_NEW_loc_BID_toward[NEWG_VEHI_NUM]={};
	...
	if(fabs(deltaX) + fabs(deltaY) < 0.01){//not start running
		if(Stmp_loc_BID_previous!= S_currentBID){
			...
			if(Stmp_loc_BID_toward - Stmp_loc_BID_previous ==1){
				...
				fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  Zero delta info %0.4f %0.4f is BAD",deltaX,deltaY);
				exit(1);
			}
		...
		fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info %0.4f %0.4f is overflow",deltaX,deltaY);
		exit(1);
		...
		if(Stmp_loc_BID_previous!= S_currentBID){//more than one street are valid, need more checks with delta infos-----------NO update with bid infos
			if(Stmp_loc_BID_previous==-1 && Stmp_loc_BID_toward ==-1){//maintains last BIDs-
				...
				if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){//master orientation
					...
					fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to RIGHT but history is LEFT  conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
					...
					fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to RIGHT but history Orient is NONE  conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}
				//leave-----S_currentBID
			}
			...
			if(Stmp_loc_BID_previous== Stmp_loc_BID_toward){//------------------no orientation is provided
				fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is no orientation is provided ");
				exit(1);
				...
				fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to RIGHT but bid-tmp=%d %d are Wired",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
				exit(1);
			}
		}
		...
		if(Stmp_loc_BID_previous!= S_currentBID){//more than one street are valid, need more checks with delta infos-----------NO update with bid infos
			if(Stmp_loc_BID_previous==-1 && Stmp_loc_BID_toward ==-1){//maintains last BIDs-
				...
				if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){
					fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to LEFT but history is RIGHT  conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
					...
					fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to LEFT but history Orient is NONE  conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}
				//leave-----S_currentBID
			}
			...
			if(Stmp_loc_BID_previous== Stmp_loc_BID_toward){//------------------no orientation is provided
				...
				fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to LEFT but bid-tmp=%d %d are Wired",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
				exit(1);
			}
		}
		...
		if(Stmp_loc_BID_previous!= S_currentBID){//more than one street are valid, need more checks with delta infos-----------NO update with bid infos
			if(Stmp_loc_BID_previous==-1 && Stmp_loc_BID_toward ==-1){
				...
				if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){
					...
					fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to UP but history is DOWN conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}else {//the first movement
					fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to UP but history Orient is NONE conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}
				//leave-----S_currentBID
			}
			...
			if(Stmp_loc_BID_previous== Stmp_loc_BID_toward){//------------------no orientation is provided
				...
				fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to UP but bid-tmp=%d %d are Wired",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
				exit(1);
			}
		}
		...
		if(Stmp_loc_BID_previous!= S_currentBID){//more than one street are valid, need more checks with delta infos-----------NO update with bid infos
			if(Stmp_loc_BID_previous==-1 && Stmp_loc_BID_toward ==-1){//BAD-----
				...
				if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){
					...
					fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to DOWN but history is UP conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}else if(S_vehicle_direction_ == NEW_ORIENT_DOWN){

				}else {//the first movement
					fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to DOWN but history Orient is NONE conflicts!!!%d %d",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
					exit(1);
				}
				//leave-----S_currentBID
			}
			...
			if(Stmp_loc_BID_previous== Stmp_loc_BID_toward){//------------------no orientation is provided
				...
				fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info is to DOWN but bid-tmp=%d %d are Wired",Stmp_loc_BID_previous,Stmp_loc_BID_toward);
				exit(1);
			}
		}
		...
		if(S_vehicle_direction_ == NEW_ORIENT_RIGHT){
			...
			fprintf(stderr,"TAVRagent::new_Supdate_DirectionInfo  delta info %0.4f %0.4f can NOT be dealt but history Orient is NONE conflicts!!!%d %d",deltaX,deltaY,Stmp_loc_BID_previous,Stmp_loc_BID_toward);
			exit(1);
		}
	}
...
/*		S_currentBID = tmp_SaveBID%Gscen_colC + 1 + tmp_SaveBID - tmp_SaveBID%Gscen_colC;
		...
		if(S_currentBID > -1) new_Supdate_JuncInfo(cvx,cvy);*/
		if(!S_junction_){
			fprintf(stderr,"\nTAVRagent::new_Supdate_JuncInfo  street info calculate beyond design%d %d %d %d",tmp_UP,tmp_DOWN,tmp_LEFT,tmp_RIGHT);
			exit(1);
		}else{//inner area????
/*			S_loc_BID_previous = S_currentBID;
	...
	fprintf(stdout,"\nTAVRagent::new_Supdate_JuncInfo finish vehi-%d bid=%d %d %d UDLR=%d %d %d %d",nodeid,S_currentBID,Stmp_loc_BID_previous,Stmp_loc_BID_toward,tmp_UP,tmp_DOWN,tmp_LEFT,tmp_RIGHT);
}
...
//	double 	innerSquarewidth = 3.2 / 1.413;
	...
	if((cvx < GXbas_LOC[sBr][sBc]+0.001) && (Orient_type == NEW_ORIENT_RIGHT)){//no RIGHT street
		return false;
	}else if((cvx > GXbas_LOC[sBr][sBc]-0.001) && (Orient_type == NEW_ORIENT_LEFT)){//no RIGHT street
		return false;
	}
 ...
 *	the threshold of radius will be less than Communication radius
 * */
void
