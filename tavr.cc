...
//	tavrh->vehicle_speed()			= 3.0 + vehicle_ip() * vehicle_ip();
	...
	case 15:
		ch->next_hop() = Address::instance().str2addr("1.0.15");break;
	case 2:
	...
	ih->saddr() = vehicle_ip();
//	ih->daddr() = Address::instance().str2addr(conf_test_AXIS_ip.c_str());
	ih->sport() = RT_PORT;
	ih->dport() = RT_PORT;
	ih->ttl_ = 10;
	...
	switch(conf_test_AXIS_ip){
	case 15:
		ih->daddr() = Address::instance().str2addr("1.0.15");break;
	case 2:
		ih->daddr() = Address::instance().str2addr("1.0.2");break;
	default:
		ih->daddr() = Address::instance().str2addr("2.0.2");break;
	}
...
//	fprintf(stdout, "\n---- ** --- pttype=%u s=%d d=%d c=%d", tavr->tavr_type(),ih->saddr(),ih->daddr(),vehicle_ip());
...
*/
	...
	if(hello_amount_ >= tmp_recv_count || triggerReplyHello(VEHICULAR_INFO_PTB_TIMEOUT))replyHello();
}
