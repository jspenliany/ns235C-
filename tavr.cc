...
//				exit(1);
		...
		if(func_Injunc(baseid,juncRadius,vehicle_position_x(),vehicle_position_y())){//inner circle
			...
			fprintf(stdout,"TAVRagent::recv_tavr_pkt second junction info 00--------Inner circle---------(01 02 03 04 should not shown more than once)");
...
/*					if(fabs(junc_row_next_list[nodeid] - junc_row_prev_list[nodeid]) == 0){
					}else{
					...
					if(fabs(junc_col_next_list[nodeid] - junc_col_prev_list[nodeid]) == 0){//from Y-axis direction movement
						fprintf(stdout,"TAVRagent::recv_tavr_pkt normal 11 delta info shows that vehicle has Y-axis delta but located at left street NO update , %0.2f %0.2f",delta_x,delta_y);
					}else if(fabs(junc_row_next_list[nodeid] - junc_row_prev_list[nodeid]) == 0){
						fprintf(stdout,"TAVRagent::recv_tavr_pkt normal 11 delta info shows that vehicle has Extreme X-axis delta and located at left street NO update  , %0.2f %0.2f",delta_x,delta_y);
					}else{
...
/*					if(fabs(delta_x) > fabs(delta_y) + 0.08){
				...
				fprintf(stdout,"TAVRagent::recv_tavr_pkt normal 21 delta info shows that vehicle NOT left or right direction  , %0.2f %0.2f",delta_x,delta_y);
			}
//------------------check-------06091018
			if(baseid-conf_scenario_colc > -1 && func_Instreet(baseid,baseid-conf_scenario_colc,laneWidth,vehicle_position_x(),vehicle_position_y(),juncRadius)){//locate street id
				if(fabs(delta_x) + fabs(delta_y) > 1.51){
...
//why two hops away between prev and next is NOT considered------1058
		...
		//----------------------------------------
			...
			//---------------to filter the condition that vehicle located at left street ,but have bigger up/down delta info
						if(fabs(junc_col_next_list[nodeid] - junc_col_prev_list[nodeid]) == 0){//from Y-axis direction movement
							fprintf(stdout,"TAVRagent::recv_tavr_pkt normal 39 delta info shows that vehicle has Y-axis delta and located at up street NO update , %0.2f %0.2f",delta_x,delta_y);
						}else if(fabs(junc_row_next_list[nodeid] - junc_row_prev_list[nodeid]) == 0){
							fprintf(stdout,"TAVRagent::recv_tavr_pkt normal 39 delta info shows that vehicle has Extreme X-axis delta but located at up street NO update  , %0.2f %0.2f",delta_x,delta_y);
						}else{
