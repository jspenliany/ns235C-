...
TAVRagent::update_vehicular_info(){
	...
	if(debug_file  && (Scheduler::instance().clock() > file_wirte_Time + 0.001)){
		...
		switch(((int)round(delta_x))){
		case 1:
			sprintf(tmpStr,"\n**%d.----right------.update_vehicular_info.\tx=%0.4f\ty=%0.4f\tv=%0.4f\tv=%d\tdx=%0.4f\tdy=%0.4f\tdx=%0.4f\tdy=%0.4f \n",conf_node_id,
					node_->X(),node_->Y(),node_->speed(),vehicle_speed_LIST_[nodeid],dst_x,dst_y,delta_x,delta_y);
			finish = true;
			break;
		case -1:
			sprintf(tmpStr,"\n**%d.----left------.update_vehicular_info.\tx=%0.4f\ty=%0.4f\tv=%0.4f\tv=%d\tdx=%0.4f\tdy=%0.4f\tdx=%0.4f\tdy=%0.4f \n",conf_node_id,
					node_->X(),node_->Y(),node_->speed(),vehicle_speed_LIST_[nodeid],dst_x,dst_y,delta_x,delta_y);
			finish = true;
			break;
		case 0:
			sprintf(tmpStr,"\n**%d.----NONE L&R------.update_vehicular_info.\tx=%0.4f\ty=%0.4f\tv=%0.4f\tv=%d\tdx=%0.4f\tdy=%0.4f\tdx=%0.4f\tdy=%0.4f \n",conf_node_id,
					node_->X(),node_->Y(),node_->speed(),vehicle_speed_LIST_[nodeid],dst_x,dst_y,delta_x,delta_y);
			finish = false;
			break;
		default:
		...
		case 1:
			if(!finish)sprintf(tmpStr,"\n**%d.----up------.update_vehicular_info.\tx=%0.4f\ty=%0.4f\tv=%0.4f\tv=%d\tdx=%0.4f\tdy=%0.4f\tdx=%0.4f\tdy=%0.4f \n",conf_node_id,
					node_->X(),node_->Y(),node_->speed(),vehicle_speed_LIST_[nodeid],dst_x,dst_y,delta_x,delta_y);
			break;
		case -1:
			if(!finish)sprintf(tmpStr,"\n**%d.-----down-----.update_vehicular_info.\tx=%0.4f\ty=%0.4f\tv=%0.4f\tv=%d\tdx=%0.4f\tdy=%0.4f\tdx=%0.4f\tdy=%0.4f \n",conf_node_id,
					node_->X(),node_->Y(),node_->speed(),vehicle_speed_LIST_[nodeid],dst_x,dst_y,delta_x,delta_y);
			break;
		case 0:
			if(!finish)sprintf(tmpStr,"\n**%d.----NONE U&D------.update_vehicular_info.\tx=%0.4f\ty=%0.4f\tv=%0.4f\tv=%d\tdx=%0.4f\tdy=%0.4f\tdx=%0.4f\tdy=%0.4f \n",conf_node_id,
					node_->X(),node_->Y(),node_->speed(),vehicle_speed_LIST_[nodeid],dst_x,dst_y,delta_x,delta_y);
			break;
		default:
			fprintf(stderr,"ERROR, TAVRagent::update_vehicular_info ....direction UP/DOWN-----%d..",vehicle_speed_LIST_[nodeid]);
...
//hard code for scale 1000
	...
	if(debug_file  && (Scheduler::instance().clock() > file_wirte_Time + 0.001)){
		...
		case DUP:
			sprintf(tmpStr,"\n**%d...from\tid=%d\tx=%d\ty=%d\tv=%d\t UP junc_pre=r%dc%d\tjunc_next=r%dc%d\tfresh=%d\ttime=%0.4f",conf_node_id,nodeid,
			...
			break;
		case DDOWN:
			sprintf(tmpStr,"\n**%d...from\tid=%d\tx=%d\ty=%d\tv=%d\t DOWN junc_pre=r%dc%d\tjunc_next=r%dc%d\tfresh=%d\ttime=%0.4f",conf_node_id,nodeid,
					vehicle_position_x_LIST_[nodeid],vehicle_position_y_LIST_[nodeid],vehicle_speed_LIST_[nodeid],
					junc_row_prev_list[nodeid],junc_col_prev_list[nodeid],junc_row_next_list[nodeid],junc_col_next_list[nodeid],
					recv_seqno_List_[nodeid],vehicle_info_updateTime_LIST_[nodeid]);
			break;
		case DLEFT:
			sprintf(tmpStr,"\n**%d...from\tid=%d\tx=%d\ty=%d\tv=%d\t LEFT junc_pre=r%dc%d\tjunc_next=r%dc%d\tfresh=%d\ttime=%0.4f",conf_node_id,nodeid,
					vehicle_position_x_LIST_[nodeid],vehicle_position_y_LIST_[nodeid],vehicle_speed_LIST_[nodeid],
					junc_row_prev_list[nodeid],junc_col_prev_list[nodeid],junc_row_next_list[nodeid],junc_col_next_list[nodeid],
					recv_seqno_List_[nodeid],vehicle_info_updateTime_LIST_[nodeid]);
			break;
		case DRIGHT:
			sprintf(tmpStr,"\n**%d...from\tid=%d\tx=%d\ty=%d\tv=%d\t RIGHT junc_pre=r%dc%d\tjunc_next=r%dc%d\tfresh=%d\ttime=%0.4f",conf_node_id,nodeid,
					vehicle_position_x_LIST_[nodeid],vehicle_position_y_LIST_[nodeid],vehicle_speed_LIST_[nodeid],
					junc_row_prev_list[nodeid],junc_col_prev_list[nodeid],junc_row_next_list[nodeid],junc_col_next_list[nodeid],
					recv_seqno_List_[nodeid],vehicle_info_updateTime_LIST_[nodeid]);
			break;
		case -17:
		case -73:
			sprintf(tmpStr,"\n**%d...from\tid=%d\tx=%d\ty=%d\tv=%d\t STOP junc_pre=r%dc%d\tjunc_next=r%dc%d\tfresh=%d\ttime=%0.4f",conf_node_id,nodeid,
					vehicle_position_x_LIST_[nodeid],vehicle_position_y_LIST_[nodeid],vehicle_speed_LIST_[nodeid],
					junc_row_prev_list[nodeid],junc_col_prev_list[nodeid],junc_row_next_list[nodeid],junc_col_next_list[nodeid],
					recv_seqno_List_[nodeid],vehicle_info_updateTime_LIST_[nodeid]);
			break;
		default:
			sprintf(tmpStr,"\n**%d.ERROR..from\tid=%d\tx=%d\ty=%d\tv=%d\tjunc_pre=r%dc%d\tjunc_next=r%dc%d\tfresh=%d\ttime=%0.4f",conf_node_id,nodeid,
					vehicle_position_x_LIST_[nodeid],vehicle_position_y_LIST_[nodeid],vehicle_speed_LIST_[nodeid],
					junc_row_prev_list[nodeid],junc_col_prev_list[nodeid],junc_row_next_list[nodeid],junc_col_next_list[nodeid],
					recv_seqno_List_[nodeid],vehicle_info_updateTime_LIST_[nodeid]);
		}
...
//		fprintf(stdout,"\nTAVRagent::recv_Hello a hello packet from %d=%hd will be filled up with %d's info",pnodeID,hello->vehicle_id(),cnodeID);
