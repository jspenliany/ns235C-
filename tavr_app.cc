...
TAVRApp::stimeout(){
	...
	if(running_){
		...
		if(debug_file && (Scheduler::instance().clock() > file_wirte_Time + 0.001)){
			sprintf(tmpStr,"\nnode %d type %d will retransmission at %0.2f + %0.2f= %0.2f",node_id,type_of_node, Scheduler::instance().clock(),timer_delay,(timer_delay + Scheduler::instance().clock()));
...
//		else fprintf(stdout,"\nAPP----COMM %d send ",node_id);
		if(type_of_node == WIRED_NODE) sprintf(tmpStr,"\nAPP----wired %d send ",node_id);
		else sprintf(tmpStr,"\nAPP----COMM %d send ",node_id);
...
TAVRApp::recv_msg(int nbytes, hdr_veh_hello *msg){
	...
	if(type_of_node == COMM_NODE && debug_file  && (Scheduler::instance().clock() > file_wirte_Time + 0.001)){
		...
		sprintf(cont,"\n**recv_hello %d-*%d---*%d---*%d--*%d--*%d**%0.2f",current_veh_num,i,vehicle_position_x_list[i],vehicle_position_y_list[i],vehicle_speed_list[i],recv_seqno_list[i],
...
/*		if((msg->vehicle_idlist()[i] < 0) || (msg->infos_tstamplist()[i] < laest_update_tstamp_list[i])){
		...
		if((msg->vehicle_idlist()[i] < 0)){
			...
			if(debug_file  && (Scheduler::instance().clock() > file_wirte_Time + 0.001)){
				sprintf(cont,"\n**recv wired %d-*%d---*%d---*%d--*%d--*%d**%0.2f",current_veh_num,i,vehicle_position_x_list[i],vehicle_position_y_list[i],vehicle_speed_list[i],recv_seqno_list[i],
																		Scheduler::instance().clock());
				TAVRApp::file_op.wirmsg_write(cont);
			}
...
//	fprintf(stdout,"\n prepare_data_for_Agent a hello packet from %hd will be filled up with %d's info",hello->vehicle_id(),node_id);
...
TAVRApp::prepare_data_for_Agent(hdr_wired_infos *reply){
	...
	if(type_of_node == COMM_NODE && debug_file  && (Scheduler::instance().clock() > file_wirte_Time + 0.001)){
		...
		TAVRApp::file_op.app_write(tmpStr);
		TAVRApp::file_op.helmsg_write("\n\n----COMM--------this round of hello recv finished---------------\n");
...
//		TAVRApp::file_op.helmsg_write("\n\n---WIRED---------this round of hello recv finished---------------\n");
	...
	for(int i=0; i< current_veh_num; i++){
		...
		if(debug_file  && (Scheduler::instance().clock() > file_wirte_Time + 0.001)){
			sprintf(tmpStr,"\n**comm prepare data %d-*%d---*%d---*%d--*%d--*%d**%0.2f",current_veh_num,i,vehicle_position_x_list[i],vehicle_position_y_list[i],vehicle_speed_list[i],recv_seqno_list[i],
																				Scheduler::instance().clock());
