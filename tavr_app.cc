...
TAVRApp::TAVRApp():running_(0),seqno_(0),tsend_timer(this),treply_timer(this){
	type_of_node = 0;
	node_id = 0;
...
TAVRApp::stimeout(){
	if(running_){
		int jitter_tmp = JITTER;
		fprintf(stdout,"\nnode %d type %d will start transmission at %0.2f",node_id,type_of_node,send_interval_ + jitter_tmp);
		tsend_timer.resched(send_interval_ + jitter_tmp);
		send_tavr_pkt();
	}
}
...
TAVRApp::send_tavr_pkt(){
	fprintf(stdout,"\n----node %d type %d send_tavr_pkt",node_id,type_of_node);
...
TAVRApp::recv_msg(int nbytes, hdr_veh_hello *msg){
	int i = 0;
	i = msg->vehicle_id();
	if(msg->hello_tstamp() < laest_update_tstamp_list[i]) return;
...
TAVRApp::recv_msg(int nbytes, hdr_wired_infos *msg){
	...
	for(; i < VEHICULAR_AMOUNT; i++){
		...
		if((msg->vehicle_idlist()[i] < 0) || (msg->infos_tstamplist()[i] < laest_update_tstamp_list[i]))continue;
