...
TAVRApp::recv_msg(int nbytes, hdr_veh_hello *msg){
	...
	if(type_of_node == COMM_NODE){
		...
		sprintf(cont,"\n**recv %d--%d----%d----%d---%d---%d**%0.2f",current_veh_num,i,vehicle_position_x_list[i],vehicle_position_y_list[i],vehicle_speed_list[i],recv_seqno_list[i],
...
/*		if((msg->vehicle_idlist()[i] < 0) || (msg->infos_tstamplist()[i] < laest_update_tstamp_list[i])){
		...
		if((msg->vehicle_idlist()[i] < 0)){
			sprintf(cont,"\n recv+-* +-* ERROR +-* +-* +-*TAVRApp::recv_msg (info) ...wired.... veh=%d LOST...",i);
