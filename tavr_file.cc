...
void
TAVRfile::reachjunc_write(char *str, int id){
	if(juncFile == NULL){
		fprintf(stderr,"error TAVRfile::reachjunc_write ..juncFile.txt create ERROR...");
		exit(1);
	}

	if(id < 0)fprintf(juncFile,"%s",str);
	else fprintf(juncFile,"%s-----------------%d",str,id);
}
...
//	from_layer = ft;
	...
	from_wired = ((ft%100)%10);
		...
		case RTR_FROM:
			...
			create_junc();
			if(juncFile == NULL){
				fprintf(stderr,"error TAVRfile::init_file ..%s create ERROR...",juncFile);
				exit(1);
			}
			break;
...
TAVRfile::clean_file(char *file_name){
	...
	for(; i< ((int)(sizeof(appLayer_info)/sizeof(char)))-6; i++){
		...
		if(i < 4){
			...
			if(from_layer == APP_FROM)appLayer_info[i] = file_name[i];
				...
				rtrLayer_info[i] = file_name[i];
				reachJunc_info[i] = file_name[i];
	...
	case RTR_FROM:
		...
		reachJunc_info[i] = '.';
		reachJunc_info[i+1] = 't';
		reachJunc_info[i+2] = 'x';
		reachJunc_info[i+3] = 't';
		reachJunc_info[i+4] = '\0';
		reachJunc_info[i+5] = '\0';break;
	case TXL_FROM:
		txlLayer_info[i] = '.';
		txlLayer_info[i+1] = 't';
		txlLayer_info[i+2] = 'x';
	...
	fprintf(stdout,"\n===========\nfile list %s %s %s %s %s %s\n",appLayer_info,txlLayer_info,rtrLayer_info,helloMSG_info,wiredMSG_info,reachJunc_info);

	switch(from_layer){
	case APP_FROM:
		if(fopen(appLayer_info,"r") != NULL) remove(appLayer_info);break;
	case RTR_FROM:
		if(fopen(rtrLayer_info,"r") != NULL) remove(rtrLayer_info);
		if(fopen(reachJunc_info,"r") != NULL) remove(reachJunc_info);break;
	case TXL_FROM:
...
TAVRfile::close(){
	...
	case APP_FROM:
		if(appFile == NULL){
			fprintf(stderr,"error TAVRfile::close ..app_layer.txt close ERROR...");
			exit(1);
		}
	...
	case TXL_FROM:
		if(txlFile == NULL){
			fprintf(stderr,"error TAVRfile::close ..txl_layer.txt close ERROR...");
			exit(1);
		}
	...
	case RTR_FROM:
		if(rtrFile == NULL){
			fprintf(stderr,"error TAVRfile::close ..rtr_layer.txt close ERROR...");
			exit(1);
		}
		fclose(rtrFile);
		if(juncFile == NULL){
			fprintf(stderr,"error TAVRfile::close ..juncFile.txt close ERROR...");
			exit(1);
		}
		fclose(juncFile);break;
	default:
	...
	case WIRED_FROM:
		if(wiredFile == NULL){
			fprintf(stderr,"error TAVRfile::close ..wired_info.txt close ERROR...");
			exit(1);
		}
		fclose(wiredFile);break;
	case 10:
		if(helloFile == NULL){
			fprintf(stderr,"error TAVRfile::close ..hello_info.txt close ERROR...");
			exit(1);
		}
...
void
TAVRfile::create_junc(){
	juncFile = fopen(reachJunc_info,"a+");
}
