...
//		Tcl& tcl = Tcl::instance();
		if (strcmp(argv[1], "node_id") == 0) {
			...
			comm_id = atoi(argv[2]);
			return (TCL_OK);
		}else if (strcmp(argv[1], "comm_interval") == 0) {
			comm_send_interval = atof(argv[2]);
			return (TCL_OK);
...
//	if(type_of_node == COMM_NODE)fprintf(stdout,"---***TAVRAppAgent::send_msg start ");
	...
	if(type_of_node == COMM_NODE){
		for(int i=0; i< current_veh_num; i++){
			...
			fresh_Degree = (curTime - vehicle_info_updateTime_LIST_[i]) / comm_send_interval;
			if(fresh_Degree -126.0001 > 0.001){
				fprintf(stderr,"error AppAgent::send_msg (info) ...COMM.... The fresh degree of veh=%d is beyond the range",i);
				exit(1);
			}

			app_msg.recv_seqno_list[i] = (int8_t)floor(fresh_Degree);
		}
		...
		for(int i=0; i< current_veh_num; i++){
			...
			fresh_Degree = (curTime - vehicle_info_updateTime_LIST_[i]) / comm_send_interval;// Alert, this interval is the comm timer, but here is common interval
			if(fresh_Degree -251.0001 +  hvh->recv_seqno_list[i]> 0.001){
				fprintf(stderr,"error AppAgent::send_msg (info) ...common.... The fresh degree of veh=%d is beyond the range",i);
				exit(1);
			}

			app_msg.recv_seqno_list[i] = hvh->recv_seqno_list[i] + (int8_t)floor(fresh_Degree);
		}
	}else{
...
//		if(wired_i == 0)fprintf(stdout,"\n comm info -id-----x----y---v--recvSEQ-list \n");
		sprintf(tmpStr,"\n comm send info -id-----x----y---v--recvSEQ-list \n");
	...
	//		if(curTime - app_->get_laest_update_tstamp_list()[i] - intervalRate*app_->get_send_interval() < 0.001){
			if(app_msg.vehicle_idlist()[i] > -1){
				...
				seqNO = (int)((ch->timestamp() - vehicle_info_updateTime_LIST_[i]) /comm_send_interval);
				...
				reply->recv_seqno_list[i] = 80;
				fprintf(stdout,"---***--error AppAgent::comm_send (info) ...COMM.... veh=%d does NOT exist----***---",i);
//				exit(1);
			}
...
//	if(node_id == 0)fprintf(stdout,"\n wired send info -id-----x----y---v--recvSEQ-list \n");
	sprintf(tmpStr,"\n %d wired send info -id-----x----y---v--recvSEQ-list \tctime=%0.4f\n",node_id,Scheduler::instance().clock());
	TAVRAppAgent::file_op.wirmsg_write(tmpStr);
	int seqNO = 0;
	for(int i=0; i< current_veh_num; i++){
//		if(curTime - app_->get_laest_update_tstamp_list()[i] - intervalRate*app_->get_send_interval() < 0.001){
