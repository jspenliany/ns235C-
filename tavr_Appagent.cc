...
//	ih->sport() = thi;
	ih->dport() = this->port();
	ih->ttl_ = 3;//since the closest wired node is at least two hops from current vehicle
	...
	if (app_ ) {
		...
		if(cmnh->ptype() == PT_TAVRHELLO || cmnh->ptype() == PT_TAVRWIRED){
			if(cmnh->ptype() == PT_TAVRHELLO){
				sprintf(tmpStr,"\n TAVRAppAgent::recv  TAVR_HELLO reach application layer!! sip=%s, cid=%d, dip=%s",
					Address::instance().print_nodeaddr(iph->saddr()),
					node_id,
					Address::instance().print_nodeaddr(iph->daddr()));
			}else if(cmnh->ptype() == PT_TAVRWIRED){

				sprintf(tmpStr,"\n TAVRAppAgent::recv  TAVR_WIRED reach application layer!! sip=%s, cid=%d, dip=%s",
					Address::instance().print_nodeaddr(iph->saddr()),
					node_id,
					Address::instance().print_nodeaddr(iph->daddr()));
			}
